...
class AlignmentStepper(Node):
    def __init__(self):
        ...
        # 🔗 整合模式（關鍵參數）
        self.declare_parameter('integrated_mode', True)              # True：由外部總控負責 Nav2/Initial Pose
        ...
        self.integrated_mode = bool(self.get_parameter('integrated_mode').value)
        ...
        if not self.integrated_mode:
            # 單機模式：由本節點自己等 Nav2 Active（可能會觸發 AMCL 初始姿態）
            self.get_logger().info('⏳ 等待 Nav2 Active …')
            self.nav.waitUntilNav2Active()
            self.get_logger().info('✅ Nav2 Active')
        else:
            # 整合模式：假設外部 bringup 已完成且 Active，這裡不再 wait/不重置
            self.get_logger().info('🔌 integrated_mode=True：跳過 waitUntilNav2Active()，避免姿態被重置')
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            # 這裡只 spin 本節點，避免對 BasicNavigator 的 node 進行二次 spin
