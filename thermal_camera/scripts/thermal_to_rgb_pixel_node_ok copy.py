...
class ThermalToRGBPixelNode(Node):
    ...
    def thermal_callback(self, msg):
        ...
        # 框的邊界（裁切不超出畫面）
        x1 = int(np.clip(pt_tl_rs[0], 0, self.rgb_width - 1))
        y1 = int(np.clip(pt_tl_rs[1], 0, self.rgb_height - 1))
        x2 = int(np.clip(pt_br_rs[0], 0, self.rgb_width - 1))
        y2 = int(np.clip(pt_br_rs[1], 0, self.rgb_height - 1))

        # 發布 [熱點x, 熱點y, 框x1, y1, x2, y2]
