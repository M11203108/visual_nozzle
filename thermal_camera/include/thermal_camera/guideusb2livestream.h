...
typedef struct
{
    float fEnvironmentIncrement;    //环温
    float fBackNearLensIncrement;   //镜筒修正量
    float fBackNearShutterIncrement;//快门修正量
    unsigned int exkf;
    short exb;
    float distance;
    float emiss;                    //发射率
    float transs;                   //透过率
    float reflectTemp;              //反射温度
}guide_measure_debugparam_t;
...
//Measure
  int guide_measure_loadcurve(); //
float guide_measure_convertsinglegray2temper(short y16, short* paramLine, guide_measure_debugparam_t* mDebugParam,int isManual);
  int guide_measure_convertgray2temper(int width,int height,float* pTempMatrix, short* y16,short* paramLine,guide_measure_debugparam_t* mDebugParam,int isManual);
  int guide_measure_deloadcurve();
